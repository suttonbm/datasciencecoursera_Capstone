...
knitr::opts_chunk$set(message=FALSE, warning=FALSE, cache=TRUE,
                      cache.path="cache/sampling_updated/")
...
```{r echo=F}
load("data/TokensVsSize.noStop.RData")
...
```{r echo=F}
tmp <- new.env()
load("data/blogs/blogs.0.001.1-gram.RData", envir=tmp)
blogs.001 <- tmp$df
load("data/blogs/blogs.0.01.1-gram.RData", envir=tmp)
blogs.01 <- tmp$df
remove(tmp)
```
...
```{r echo=F}
tmp <- new.env()
load("data/blogs/blogs.0.001.2-gram.RData", envir=tmp)
blogs.001.bi <- tmp$df
load("data/blogs/blogs.0.01.2-gram.RData", envir=tmp)
blogs.01.bi <- tmp$df
load("data/blogs/blogs.0.001.3-gram.RData", envir=tmp)
blogs.001.tri <- tmp$df
load("data/blogs/blogs.0.01.3-gram.RData", envir=tmp)
blogs.01.tri <- tmp$df
remove(tmp)
```
```{r}
