...
knitr::opts_chunk$set(message=FALSE, warning=FALSE, cache=TRUE,
                      cache.path='cache/2016-07-08-tokenizing_aside/')
...
```{r echo=F}
library(ggplot2)
ggplot(rbind(words.df, mc.df),
       aes(x=file.lines, y=perf, color=method)) +
  geom_point()
ggplot(rbind(words.df, mc.df),
       aes(x=file.size, y=perf, color=method)) +
  geom_point()
```
...
parallelTask.qeda <- function(corpus, n=1) {
  ...
  tokens <- quanteda::tokenize(corpus, what='word', ngrams=n, simplify=TRUE, verbose=FALSE)
  r <- quanteda::dfm(tokens, verbose=FALSE)
