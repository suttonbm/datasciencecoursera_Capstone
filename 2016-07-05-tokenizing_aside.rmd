...
```{r echo=F}
library(ggplot2)
ggplot(rbind(words.df, mc.df), aes(x=file.lines, y=perf, color=method)) + geom_point()
ggplot(rbind(words.df, mc.df), aes(x=file.size, y=perf, color=method)) + geom_point()
```
...
```{r echo=F}
ggplot(rbind(nlp.2.df, nlp.3.df, words.df), aes(x=file.lines, y=perf, color=method)) + geom_point()
```
...
```{r echo=F}
ggplot(rbind(rweka.1.df, rweka.2.df, rweka.3.df, words.df), aes(x=file.lines, y=perf, color=method)) + geom_point()
```
...
```{r echo=F}
parallelTask.qeda <- function(corpus, n=1) {
  ncores <- detectCores() - 1
  options(mc.cores = ncores)
  cl <- makeCluster(ncores)
  registerDoParallel(cl)
  
  tokens <- quanteda::tokenize(corpus, what='word', ngrams=n, simplify=TRUE)
  r <- quanteda::dfm(tokens)
  
  stopCluster(cl)
  r
}

doEvaluate.qeda <- function(method.name, n=1) {
  times <- list()
  for (id in ids) {
    c <- quanteda::corpus(corpora[[id]])
    t <- system.time({
      parallelTask.qeda(c, n)
    })[['elapsed']]
    times[[id]] <- t
  }
...
```{r echo=F}
ggplot(rbind(qeda.1.df, qeda.2.df, qeda.3.df, words.df), aes(x=file.lines, y=perf, color=method)) + geom_point()
```
...
```{r echo=F}
final.data <- rbind(words.df, mc.df,
                    nlp.2.df, nlp.3.df,
                    rweka.1.df, rweka.2.df, rweka.3.df,
                    qeda.1.df, qeda.2.df, qeda.3.df)
write.table(final.data, file="data/desktop-tokens-perf.tab")
```
